 ...
 *
 */
function diy_get_menu($parent_id, $navType = 'main') {
    $navs = F("left_nav_" . $navType . "_" . $parent_id);
    if (empty($navs)) {
        $navs = _diy_get_menu_datas($parent_id, $navType);
    }
    return $navs;
}

function _diy_get_menu_datas($parent_id, $navType) {
    $nav_obj = M("Nav");
    $oldid = $navType;
    $id = intval($navType);
    $id = empty($id) ? "main" : $id;
    ...
    //---- 顶级菜单 ----
    $navsTemps = $nav_obj
        ->where(array('cid' => $id, 'status' => 1, 'id' => $parent_id))
        ->order(array("listorder" => "ASC"))
        ->select();
    $navs['parent'] = _transform_navs_href($navsTemps);
    $navs['parent'] = $navs['parent'][0];
    //---- 顶级菜单的子菜单 ----
    $navsTemps = $nav_obj
        ->where(array('cid' => $id, 'status' => 1, 'parentid' => $parent_id))
        ->order(array("listorder" => "ASC"))
        ->select();
    $navs['child'] = _transform_navs_href($navsTemps);

    F("left_nav_" . $oldid . "_" . $parent_id, $navs);
    return $navs;
}

//---- 转换href参数 ----
function _transform_navs_href($navs) {
